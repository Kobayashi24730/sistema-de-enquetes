...
export default function Editar() {
    ...
    useEffect(()  => {
        async function fetchEnquetes() {
            try {
                ...
                const enquete = response.data;
                setFormData({
                    title: enquete.title,
                    category: enquete.category,
                    expiresAt: enquete.expires_at
                });

                if (enquete.options && enquete.options.length > 0) {
                    const optionTexts = enquete.options.map(option => option.option_text);
                    setOptions(optionTexts);
                }
            } catch (err) {
                console.error("Erro ao carregar enquete: ", err);
                setError("Não foi possivel carregar os dados da enquete para edição.");
            } finally {
                setLoading(false);
            }
        }
        ...
        return (
            <div className="min-h-[60vh] flex items-center justify-center p-4">
                <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-lg border border-gray-100 text-center flex flex-col items-center gap-5">
                    <div>
                        <p className="text-sm text-gray-500 mt-1">Por favor, faça login para editar uma enquete.</p>
                    </div>
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.put(`/enquetes/update?id=${id}`, {
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-bold tracking-tight">Editar enquete</h1>
